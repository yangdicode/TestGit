package mmo.nurturance.ui.multi
{
	...
	public class NurMultiPage0_2 implements INurMultiPage0SubPage
	{
		private var _mcContainer:MovieClip;
		private var _data:Array;
		
		public function NurMultiPage0_2()
		{
		}
		...
		public function init(mcContainer:MovieClip):void
		{
			...
			_data = NurGameManager.instance.gameStateProxy.getDoingGameState();
			
			for(var i:int = 0; i < _data.length; i++){
				var mcItem:MovieClip = SceneDomainResLoader.getResByName("mmo.nurturance.sysui.MultiStoryItem");
				var gameState:NurUserGameState = _data[i];
				initItem(gameState, mcItem);
				addItem(i, mcItem);
			}
		}
		
		private function initItem(gameState:NurUserGameState, mcItem:MovieClip):void{
			var gameDefine:NurGameDefine = NurDefinesProvider.getGameById(gameState.gameId);

			MovieClip(mcItem["mcNotice"]).gotoAndStop(3);
			MovieClip(mcItem["mcStart"]).gotoAndStop(2);
			...
			TextField(mcItem["mcNotice"]["txtMyChapterNum"]).text = String(gameState.finishChapter.length);
			TextField(mcItem["mcNotice"]["txtTotaChapterNum"]).text = String(NurDefinesProvider.getChaptersByGameId(gameDefine.id).length);
			TextField(mcItem["mcNotice"]["txtMyEndingNum"]).text = String(gameState.finishEnding.length);
			TextField(mcItem["mcNotice"]["txtTotalEndingNum"]).text = String(NurDefinesProvider.getEndingsByGameId(gameDefine.id).length);
